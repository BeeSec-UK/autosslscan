...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-d", "--dir", dest="input_directory", required=True, help="Path to the directory containing Nmap XML files")
    parser.add_argument("-o", "--output", dest="output_directory", required=True, help="Path to the output directory")
    parser.add_argument("-t", "--threads", dest="num_threads", type=int, default=10, help="Number of threads for parallel execution")
...
def process_results(results, output_folders, vuln_folder):
    for result in results:
        ...
        if scan_output and len(temp) > 4:
            ...
            check_medium_strength_ciphers(scan_output, f"{vuln_folder}/Medium_Strength_Cipher_Suites.txt", ip, port, output_folders)
            ...
            check_self_signed_certificate(scan_output, f"{vuln_folder}/Self_Signed_Certificate.txt", ip, port, output_folders)
...
def process_report(report, output_folders, num_threads, verbose=False):
    starttls_services = starttls_services_template.copy()
    ssl_services = []
...
def check_legacy_protocols(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
...
def check_tls_v1_3_disabled(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    for line in scan_output.splitlines():
        parts = line.split()
...
def check_certificate_expiry(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    valid_from = None
    valid_until = None
    subject = None
...
def check_signed_cert_rsa_keylength(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    for line in scan_output.splitlines():
...
def check_tls_fallback(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    for line in scan_output.splitlines():
...
def check_untrusted_certificate(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    for line in scan_output.splitlines():
...
def check_medium_strength_ciphers(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    for line in scan_output.splitlines():
...
def check_null_ciphers(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    for line in scan_output.splitlines():
        if "NULL" in line:
            with open(result_path, 'a') as f:
...
def check_ssl_wildcard(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
...
def check_self_signed_certificate(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    subject = None
    issuer = None
...
def check_wrong_hostname(scan_output: str, result_path: str, host: str, port: str, output_folders: dict[str,str]) -> None:
    scan_output = remove_ansi_escape_sequences(scan_output)
    hostname = host
...
def generate_word_report(output_folders, vulnerabilities):
    try:
        ...
        print(f"{SYMBOLS['plus']} Word report generated: {os.path.join(output_folders['sslscan_folder'], 'SSL_Scan_Report.docx')}")
        
    except ImportError:
        print(f"{SYMBOLS['cross']} Could not generate Word document. Please install python-docx package: pip install python-docx")
...
def main():
    ...
    xml_files = [os.path.join(input_directory, file) for file in os.listdir(input_directory) if file.endswith(".xml")]
    ...
    # Produce XML results
    vulnerabilities = consolidate_xml_results(output_folders["vuln_output"], output_folders["starttls_vuln_output"], output_folders["sslscan_folder"])

    # Generate Word document if requested
    if generate_word:
        generate_word_report(output_folders, vulnerabilities)

    print(f"{SYMBOLS['plus']} Results consolidated in {os.path.join(output_folders['sslscan_folder'], 'Big_Final_Results.txt')}")
